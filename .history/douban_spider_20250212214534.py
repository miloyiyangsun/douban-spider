...
class DoubanBookSpider(scrapy.Spider):
    ...
    def __init__(self, book_name=None, book_id=None, *args, **kwargs):
        super(DoubanBookSpider, self).__init__(*args, **kwargs)
        self.book_name = book_name
        self.book_id = book_id  # 允许直接指定ID
        self.comments = []

    def start_requests(self):
        if self.book_id:  # 如果已有ID直接请求评论页
            for req in self.generate_comment_requests():
                yield req
        else:  # 否则走搜索流程
            # 添加URL编码验证
            encoded_name = quote(self.book_name)
            if "%" not in encoded_name:
                self.logger.error("书名编码失败")
                return

            search_url = f"https://book.douban.com/j/subject_suggest?q={encoded_name}"
            yield scrapy.Request(
                url=search_url,
                callback=self.parse_search,
                headers=self.get_random_headers(),
                meta={"dont_retry": True},  # 搜索请求不重试
            )

    def parse_search(self, response):
        # 解析搜索结果的JSON获取准确book_id
        try:
            print(f"搜索响应内容: {response.text}")  # 查看实际返回内容
...
def run_spider(book_name):
    ...
    process.crawl(DoubanBookSpider, book_name=book_name)
    process.start()


if __name__ == "__main__":
    book_name = "活着"  # 直接使用书名
    run_spider(book_name)

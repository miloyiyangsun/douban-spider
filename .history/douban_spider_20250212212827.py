    ...
    def parse_comments(self, response):
        ...
        for item in comment_items:
            ...
            if comment:
                ...
                # 提取评论者信息
                user = item.find("a", class_="name").get_text(strip=True)
                # 提取评分
                rating_span = item.find(
                    "span", class_=lambda x: x and x.startswith("allstar")
                )
                rating = rating_span["class"][0] if rating_span else "no_rating"
                # 提取时间
                time = item.find("span", class_="date").get_text(strip=True)
